...
download_pubs = function(year, destdir, name_stem = "RePORTER_PUB_C_FY"){
...
destdir = "downloaded_data/pubs/"
purrr::map(1980:2022, ~ download_pubs(., destdir = destdir))
...
download_publink = function(year, destdir, name_stem = "RePORTER_PUBLNK_C_FY"){
  
  for (i in year){
    
    file_url = paste0("https://exporter.nih.gov/CSVs/final/RePORTER_PUBLNK_C_FY", 
                      i, ".zip")
...
purrr::map(1980:2022, ~ download_publink(., destdir = destdir))
